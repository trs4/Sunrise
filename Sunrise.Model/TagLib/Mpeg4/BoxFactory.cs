...
public static class BoxFactory
{
    ...
    private static Box CreateBox(TagLib.File file, BoxHeader header, BoxHeader parent, IsoHandlerBox handler, int index)
    {
        ...
        if (parent.BoxType == BoxType.Stsd && parent.Box is IsoSampleDescriptionBox parentBox && index < parentBox.EntryCount)
        {
            if (handler is not null && handler.HandlerType == BoxType.Soun)
                return new IsoAudioSampleEntry(header, file, handler);
            else if (handler is not null && handler.HandlerType == BoxType.Vide)
                return new IsoVisualSampleEntry(header, file, handler);
            else if (handler is not null && handler.HandlerType == BoxType.Alis)
            {
                ...
                return new UnknownBox(header, file, handler); // This could be anything, so just parse it
            }
            else
    ...
    public static Box CreateBox(TagLib.File file, long position, IsoHandlerBox? handler)
        => CreateBox(file, position, BoxHeader.Empty, handler, -1);
    ...
    public static Box CreateBox(TagLib.File file, BoxHeader header, IsoHandlerBox? handler)
        => CreateBox(file, header, BoxHeader.Empty, handler, -1);
