...
/// <summary>This class extends <see cref="Tag" /> to provide support for processing Apple "ilst" tags</summary>
public class AppleTag : Tag, IEnumerable<Box>
{
    /// <summary>Contains the ISO meta box in which that tag will be stored</summary>
    private readonly IsoMetaBox? _metaBox;

    /// <summary>Contains the ILST box which holds all the values</summary>
    private readonly AppleItemListBox? _listBox;

    /// <summary>Constructs and initializes a new instance of <see cref="AppleTag" /> for a specified ISO user data box</summary>
    /// <param name="box">A <see cref="IsoUserDataBox" /> from which the tag is to be read</param>
    public AppleTag(IsoUserDataBox box)
    {
        ArgumentNullException.ThrowIfNull(box);
        _metaBox = (IsoMetaBox)box.GetChild(BoxType.Meta);
        ...
        _listBox = (AppleItemListBox)_metaBox.GetChild(BoxType.Ilst);

        if (_listBox is null)
        {
            _listBox = new AppleItemListBox();
            _metaBox.AddChild(_listBox);
        }
    }

    /// <summary>Gets and sets whether or not the album described by the current instance is a compilation</summary>
    /// <value>A <see cref="bool" /> value indicating whether or not the album described by the current instance is a compilation</value>
    /// <remarks>This property is implemented using the "cpil" data box</remarks>
    public bool IsCompilation
    {
        ...
        set => SetData(BoxType.Cpil, new ByteVector((byte)(value ? 1 : 0)), (uint)AppleDataBox.FlagType.ForTempo);
    }

    /// <summary>Gets all data boxes that match any of the provided types</summary>
    /// <param name="types">A <see cref="T:System.Collections.Generic.IEnumerable`1" /> object enumerating a list of box types to match</param>
    /// <returns>A <see cref="T:System.Collections.Generic.IEnumerable`1" /> object enumerating the matching boxes</returns>
    public IEnumerable<AppleDataBox> DataBoxes(IEnumerable<ByteVector> types)
    {
        ...
        foreach (Box box in _listBox?.Children ?? [])
        {
            foreach (ByteVector v in types)
            {
                ...
                {
                    if (dataBox is AppleDataBox adb)
                        yield return adb;
                }
            }
        }
    ...
    /// <summary>Gets all data boxes that match any of the provided types</summary>
    /// <param name="types">A <see cref="ByteVector[]" /> containing list of box types to match</param>
    /// <returns>A <see cref="T:System.Collections.Generic.IEnumerable`1" /> object enumerating the matching boxes</returns>
    public IEnumerable<AppleDataBox> DataBoxes(params ByteVector[] types)
        => DataBoxes(types as IEnumerable<ByteVector>);

    /// <summary>Gets all custom data boxes that match the specified mean and name pair</summary>
    /// <param name="mean">A <see cref="string" /> object containing the "mean" to match</param>
    /// <param name="name">A <see cref="string" /> object containing the name to match</param>
    public IEnumerable<AppleDataBox> DataBoxes(string mean, string name)
    {
        ...
        foreach (Box box in _listBox?.Children ?? [])
        {
            ...
            AppleAdditionalInfoBox meanBox = (AppleAdditionalInfoBox)box.GetChild(BoxType.Mean);
            AppleAdditionalInfoBox nameBox = (AppleAdditionalInfoBox)box.GetChild(BoxType.Name);
    ...
    /// <summary>Gets all text values contained in a specified box type</summary>
    /// <param name="type">A <see cref="ByteVector" /> object containing the box type to match</param>
    /// <returns>A <see cref="string[]" /> containing text from all matching boxes</returns>
    ...
    /// <summary>Sets the data for a specified box type to a collection of boxes</summary>
    /// <param name="type">A <see cref="ByteVector" /> object containing the type to add to the new instance</param>
    /// <param name="boxes">A <see cref="AppleDataBox[]" /> containing boxes to add for the specified type</param>
    public void SetData(ByteVector type, AppleDataBox[] boxes)
    {
        ...
        _listBox?.AddChild(box2);

        foreach (AppleDataBox b in boxes)
            box2.AddChild(b);
    }

    /// <summary>Sets the data for a specified box type using values from a <see cref="ByteVectorCollection" /> object</summary>
    /// <param name="type">A <see cref="ByteVector" /> object containing the type to add to the new instance</param>
    /// <param name="data">A <see cref="ByteVectorCollection" /> object containing data to add for the specified type</param>
    /// <param name="flags">A <see cref="uint" /> value containing flags to use for the added boxes</param>
    ...
    public void SetText(ByteVector type, string?[] text)
    {
        if (text is null)
        {
            _listBox?.RemoveChild(FixId(type));
            return;
        }
    ...
    /// <summary>Clears all data for a specified box type</summary>
    /// <param name="type">A <see cref="ByteVector" /> object containing the type of box to remove from the current instance</param>
    public void ClearData(ByteVector type) => _listBox?.RemoveChild(FixId(type));

    /// <summary>Detaches the internal "ilst" box from its parent element</summary>
    public void DetachIlst() => _metaBox?.RemoveChild(_listBox);

    /// <summary>Gets the text string from a specific data box in a Dash (----) atom</summary>
    ...
    {
        var dataBox = GetDashAtoms(meanString, nameString);
        return dataBox?.Text;
    }

    /// <summary>
    /// Sets a specific strings in Dash (----) atom.  This method updates
    /// and existing atom, or creates a new one.  If an empty data string is
    ...
    public void SetDashBox(string meanString, string nameString, string dataString)
    {
        ...
        // If we did find a data_box and we have an empty data string we should
        // remove the entire dash box
        if (dataBox is not null && string.IsNullOrEmpty(dataString))
        {
            var dashBox = GetParentDashBox(meanString, nameString);
            dashBox?.ClearChildren();
            _listBox?.RemoveChild(dashBox);
            return;
        }
        ...
        else
        {
            ...
            _listBox?.AddChild(wholeBox);
        }
    }

    /// <summary>Gets the AppleDataBox that corresponds to the specified mean and name values</summary>
    /// <param name="meanString">String specifying text for mean box</param>
    /// <param name="nameString">String specifying text for name box</param>
    ...
    private AppleDataBox? GetDashAtoms(string meanString, string nameString)
    {
        foreach (Box box in _listBox?.Children ?? [])
        {
            ...
            var meanBox = (AppleAdditionalInfoBox)box.GetChild(BoxType.Mean);
            var nameBox = (AppleAdditionalInfoBox)box.GetChild(BoxType.Name);

            if (meanBox is null || nameBox is null || meanBox.Text != meanString || nameBox.Text != nameString)
            {
            }
            else
    ...
    /// <summary>Returns the Parent Dash box object for a given mean/name combination</summary>
    /// <param name="meanString">String specifying text for mean box</param>
    /// <param name="nameString">String specifying text for name box</param>
    ...
    {
        foreach (var box in _listBox?.Children ?? [])
        {
            if (box.BoxType != BoxType.DASH)
                continue;
        ...
        return null; // If we haven't returned the found box yet, there isn't one, return null
    }
    ...
    /// <returns>A fixed <see cref="ReadOnlyByteVector" /> or <see langword="null" /> if the ID could not be fixed</returns>
    ...
    /// <summary>Gets an enumerator for enumerating through the tag's data boxes</summary>
    public IEnumerator<Box> GetEnumerator() => _listBox!.Children.GetEnumerator();

    IEnumerator IEnumerable.GetEnumerator() => _listBox!.Children.GetEnumerator();

    /// <summary>Gets the tag types contained in the current instance</summary>
    public override TagTypes TagTypes => TagTypes.Apple;

    /// <summary>Gets and sets the title for the media described by the current instance</summary>
    /// <value>
    ///    A <see cref="string" /> object containing the title for
    ///    the media described by the current instance or <see
    ///    langword="null" /> if no value is present
    /// </value>
    /// <remarks>This property is implemented using the "@nam" data box</remarks>
    ...
    public override string?[] Genres
    {
        get
        {
            ...
            foreach (AppleDataBox box in DataBoxes(BoxType.Gnre))
            {
                ...
                ushort index = box.Data?.ToUShort(true) ?? 0;
    ...
    public override uint Year
    {
        get
        {
            ...
            {
                if (box.Text != null && (uint.TryParse(box.Text, out uint value) || uint.TryParse(box.Text.Length > 4 ? box.Text.Substring(0, 4) : box.Text, out value)))
                    return value;
            }
    ...
    /// <summary>Gets and sets the MusicBrainz ArtistID</summary>
    /// <value>
    ...
    public override IPicture[] Pictures
    {
        get
        {
            ...
            {
                var picture = new Picture(box.Data)
                {
        ...
        set
        {
            ...
            var boxes = new AppleDataBox[value.Length];
    ...
    /// <value><see langword="true" /> if the current instance does not any values. Otherwise <see langword="false" /></value>
    public override bool IsEmpty => !_listBox!.HasChildren;

    /// <summary>Clears the values stored in the current instance</summary>
    public override void Clear() => _listBox?.ClearChildren();
}
