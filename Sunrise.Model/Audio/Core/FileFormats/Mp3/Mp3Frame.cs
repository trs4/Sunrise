...
/// <summary>Represents an MP3 Frame</summary>
public class Mp3Frame
{
    ...
    /// <summary>Constructs an MP3 frame</summary>
    private Mp3Frame() { }

    /// <summary>Reads an MP3 frame from a stream</summary>
    ...
    public static Mp3Frame? LoadFromStream(Stream input, bool readData)
    {
        ...
        var frame = new Mp3Frame { FileOffset = input.Position };
        byte[] headerBytes = new byte[4];
        int bytesRead = input.Read(headerBytes, 0, headerBytes.Length);

        if (bytesRead < headerBytes.Length)
            return null; // reached end of stream, no more MP3 frames
    ...
    private static bool IsValidHeader(byte[] headerBytes, Mp3Frame frame)
    {
        if ((headerBytes[0] == 0xFF) && ((headerBytes[1] & 0xE0) == 0xE0))
        {
            ...
            if (frame.BitRateIndex == 15) // invalid index
                return false;
    ...
    /// <summary>Sample rate of this frame</summary>
    public int SampleRate { get; private set; }

    /// <summary>Frame length in bytes</summary>
    public int FrameLength { get; private set; }

    /// <summary>Bit Rate</summary>
    public int BitRate { get; private set; }

    /// <summary>Raw frame data (includes header bytes)</summary>
    ...
    /// <summary>The number of samples in this frame</summary>
    public int SampleCount { get; private set; }

    /// <summary>The channel extension bits</summary>
    public int ChannelExtension { get; private set; }

    /// <summary>The bitrate index (directly from the header)</summary>
    public int BitRateIndex { get; private set; }

    /// <summary>Whether the Copyright bit is set</summary>
    public bool Copyright { get; private set; }

    /// <summary>Whether a CRC is present</summary>
    public bool CrcPresent { get; private set; }

    /// <summary>Not part of the MP3 frame itself - indicates where in the stream we found this header</summary>
